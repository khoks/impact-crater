...
| `v2` | Mobile + multi-agent + conversational + generated music | (I-4, created when v2 opens) | queued |
...
The thinnest end-to-end slice that proves the core loop: *user uploads media → AI curates → user reviews preview → user approves publish*. **One artifact (Story Video), one platform (YouTube), one routing default (remote-first).**
...
| M6 | Agentic refinement + orchestrator second-guess | [E-2.7](../../project/epics/E-2.7-agentic-refinement-and-second-guess.md) | XL |
...
## v2 — Mobile + multi-agent (~3-4 months)

v2 adds the mobile companion app and refactors the orchestrator into a multi-agent harness for the conversational features that don't fit in a single-orchestrator tool-call loop.
...
v2 closes when the product is usable on mobile + the conversational-refinement and generated-music capabilities ship.
...
- The v2 milestone count (4) and ordering.
