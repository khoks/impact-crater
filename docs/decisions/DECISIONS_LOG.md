...
### D-053 — Feedback-driven curation fixes from the SW-US-trip job (coverage, balance, video, specialness, cast, feedback-UI)

- **Status:** accepted
- **Date:** 2026-06-29
- **Context:** Inspecting the real 1,663-asset SW-US-trip job (snapshot `b4b73c7b1fe044b7`) against its media — and submitting 8 per-decision feedback items via the in-app loop — found a polished 108s arc (conf 0.85) with systematic gaps, all rooted in Stage 4 and the Stage 5 judge having no structured brief intent: Las Vegas (a named destination) dropped entirely (all 37 NV-area shots cut, 32 by `rank_below_target_size`, incl. a good Hoover Dam landmark); **zero video** selected for a "highlight video" despite 58 strong video scenes reaching the judge (which couldn't tell video from photo); a **monotonous climax** of six near-identical family-at-sunset shots; **Bryce under-covered** (largest cluster) vs Grand Canyon over-covered; the **specialness** signal computed but unused in selection; the **trip cast over-classified** (81 "group"); a **backwards opener**; and a **feedback modal that hung forever** on submit (html-to-image with no timeout, blocking the POST).
- **Decision:** (1) Make curation brief-aware via a Stage-0.5 brief parse → named-destination coverage reservation in Stage 4 → judge coverage requirement (ADR-0017, S-2.10.5). (2) Use specialness in Stage 4 (4-term score + quality-floor rescue + tie-breaks; S-2.10.2, implemented). (3) Give the judge explicit MOTION/STILL candidate tags + a rewritten prompt enforcing video share, intra-section variety, people/landscape + location balance, and a gated strict-forward opener (S-2.10.3, implemented). (4) Replace the global top-K with a capture-day stratified budget seeded by the coverage must-keeps (S-2.10.6, deferred, A/B-gated). (5) Tighten cast group/crowd (conjunctive appearance+day gate landed; merge pass + detection-confidence + insightface default pending; S-2.10.4). (6) Fix the feedback hang + complete inspect diagnostics — video thumbnails, drop-card scores (S-2.10.1, implemented). (7) Bump routing to Opus 4.8 / Sonnet 4.6 (S-2.10.7). (8) Replace caption-then-embed with a local image embedder as the F2/F3 root-cause fix (ADR-0018, S-2.10.8, v1).
- **Alternatives considered:** Split the judge into shot-list + ordering or add a second/re-rank pass (rejected — judge not overloaded at ~621 candidates; doubles Tier-L cost; can't reconsider a bad selection — the defect was missing input, not capacity). A deterministic post-judge validate-and-reprompt repair loop for the caps (deferred — prompt-only first, measure violation rate, revisit). Lowering the specialness rescue to 0.72 to catch Hoover Dam (rejected — that case is a coverage problem, not a quality-floor one; rescue stays 0.75).
- **Consequences:** New Epic E-2.10 + 8 stories; ADR-0017 (accepted) amends ADR-0011's Stage-4/Stage-5 sections; ADR-0018 (proposed). Stage 4 selection now shifts every job (specialness 20% of score), validated against the known-good arc; the riskier stratified budget ships behind its own A/B PR. S-2.9.20 (Vegas investigation) resolved: cause identified, fix tracked as S-2.10.5.
- **Linked ADRs / items:** ADR-0017, ADR-0018, ADR-0011, E-2.10, S-2.10.1–8, S-2.9.20, A-023.

### D-054 — Round-2 SW-US-trip output feedback: snappier/fuller edits + queued polish & observability

- **Status:** accepted
- **Date:** 2026-06-30
- **Context:** After the E-2.10 fixes, the user reviewed two re-renders and gave 10 specific notes: edits are too slow and too thin (5–8s held photos incl. an 8000ms opener; a jerky ~1s video clip; whole sub-locations/times within a park skipped; one viewpoint — Horseshoe Bend — given 4 slots; a 1–2 min video too short for a long trip), plus four feature asks (an opt-in AI title/splash card; an occasional 0.5s burst-montage for dense same-backdrop clusters; music mood/tempo/section-aware matching; and job-/phase-level feedback + a much richer live module/decision progress view at the depth of the pipeline infographic).
- **Decision:** Grouped the 10 notes A–D and implemented **Group A (S-2.11.1)** this session — Stage 6 hard per-clip duration band (photos 1–3s, never stretched to fill; video 2s–natural), Stage 4 min-video eligibility (<2s ineligible), and a judge prompt that drives ~40–60 snappy clips covering EVERY distinct location+time with a ~2–3 per-viewpoint cap (judge max_tokens 4096→8192). Queued the rest as **E-2.11** stories: S-2.11.2 job/phase feedback (#9), S-2.11.3 richer live module/decision viz (#10), S-2.11.4 burst-montage (#5), S-2.11.5 opt-in AI title/splash card (#7), S-2.11.6 music mood/section matching (#8). **Splash-card image generation = remote image-gen API** (per ADR-0007's remote abstraction) rather than a local model — simplest, no local weights, small per-image cost within quota; chosen by the user.
- **Alternatives considered:** Cap per-clip but keep total ≤ user target by selecting fewer clips (rejected — that's the "too thin" complaint); auto-extend the target for big trips (rejected — respect the user's chosen duration, pack it densely instead). Local image-gen for the splash card (rejected by user — heavy GPU dependency; remote API chosen). A hard per-clip MIN of 1.5s (rejected — it forced short videos over their target and would fight the future burst-montage; lowered to a 1s safety floor, the MAX is the real fix).
- **Consequences:** New Epic E-2.11 + S-2.11.1–6; S-2.11.1 implemented + unit-tested (406 pass) and Chrome-re-render-verified; music-video beat-snap kept its legacy linear scale (the per-clip band is standard-mode only). Group A committed af83d0c.
- **Linked ADRs / items:** E-2.11, S-2.11.1–6, ADR-0007, E-2.10/D-053.

### D-055 — Splash-card title from a cheap Tier-S LLM op reusing the structured-text primitive (heuristic fallback)

- **Status:** accepted
- **Date:** 2026-06-30
- **Context:** The opt-in AI title/splash card (S-2.11.5) derived its title — when the user didn't type one — from a mechanical heuristic (`_derive_title`): take the brief's first clause, drop filler words, title-case the first 5 words. For a brief like *"A highlight video of our early-April trip through zion, bryce, horseshow bend, grand canyon, and las vegas"* this produced **"Early-April Through Zion Bryce Horseshow"** — awkward, and it carried the user's "horseshow" typo verbatim onto the rendered card.
- **Decision:** Add a cheap Tier-S `generate_title_text` router op (`google` / `gemini-2.5-flash`, `max_tokens` 64, temp 0.5) that reads the brief + modal year and returns a clean 2–5 word human title, fixing obvious place-name misspellings. It **reuses the existing structured-text primitive** — the `LLMClient.parse_user_brief` client method with a one-field `{title}` JSON schema — rather than adding a new protocol method, so no provider-client changes were needed. Cached on `sha256(brief, year)`; empty outputs are not cached (a transient blank re-tries next run). The old heuristic is retained as the fail-soft fallback chain (LLM → `_clean_title` → `_derive_title` → "Our Trip"), and an explicit user `title_text` short-circuits the LLM call entirely.
- **Alternatives considered:** Add a generic `generate_text` / `complete` method to the `LLMClient` protocol (rejected — every provider client would need a new implementation for a one-field text op the structured-text path already covers). Keep and just harden the heuristic (rejected — no amount of word-filtering fixes typos or yields a genuinely evocative title). Route through Tier-M for quality (rejected — a 2–5 word title is well within Tier-S Gemini Flash; keep it cheap, one call per opted-in job).
- **Consequences:** New story S-2.11.7 (P3 polish under E-2.11); new routing op + prompt (`prompts/generate_title_text/`); `PIPELINE_DATAFLOW.md` Stage 6.x gained an AI-prompt title-text step. Establishes the pattern for future cheap one-field text ops — reuse `parse_user_brief` + a tiny JSON schema instead of widening the client protocol. Merged in PR #69. Future enrichment (A-026): feed explicit top-locations once geocoding / named-destination parse lands (S-2.10.5).
- **Linked ADRs / items:** S-2.11.7, S-2.11.5, E-2.11, ADR-0007, ADR-0009, A-026.

### D-056 — Open-ended refinement via two shared curation levers (E-2.12)

- **Status:** accepted
- **Date:** 2026-07-01
- **Context:** The refinement phase could only re-run the judge with a brief addendum, so it couldn't honour requests like "insert more photos of Las Vegas", "don't drop Vegas even if low quality", "make the beginning clips 50% longer and shrink the photos around them", or "photo duration should rise with the song's tempo". The user asked for prompt engineering that lets the agent *think* from the request + the analysis present (and re-analyse only when needed) and regenerate the video.
- **Decision:** Build TWO shared curation levers used by BOTH initial planning and refinement, then make the refinement tools thin wrappers over them. (1) **PlanDirective** (ADR-0019) — the single duration/positional/tempo shaping contract Stage 6 consumes and refinement emits (default no-op; positional emphasis, tempo-from-music, soft-align). (2) **ReservationSet** (S-2.10.5) — a source-agnostic must-keep set through Stage 4 (destination coverage = the Vegas fix; also the refinement force-include path). The refinement planner is one richly-contextualised structured call → a `RefinementOutcome` combining directive_patch (pure pacing → re-run Stage 6 only) / reserve_destinations|refs (→ re-Stage-4 + re-judge) / brief_addendum (→ re-judge) / explanation; `execute_refinement` reconstructs context from source sidecars (+ LLM cache) and renders a **child snapshot**.
- **Alternatives considered:** A full multi-turn tool-use loop (deferred — one well-contextualised call delivers the examples and matches the existing second-guess/refine pattern; the levers make each "tool" a thin wrapper). Hand-editing clip durations in the refinement layer (rejected — would diverge initial vs refined render paths; the directive keeps them identical). Re-ingesting media on refine (rejected — sidecars + LLM cache reconstruct the context without a 25-min re-ingest).
- **Consequences:** New Epic E-2.12 (S-2.12.1/2/3) + the S-2.10.x backlog landed; ADR-0019 (PlanDirective) + ADR-0020 (cast merge + stratified budget). One reservation-merge path (destination / refinement / feedback) and one shaping contract, so a fourth consumer (A-023 feedback) reuses them for free. Verified live: pacing AND coverage refinements each re-rendered a child snapshot end-to-end. The model bump surfaced that Opus 4.8 rejects `temperature` (fixed). Follow-up: refinement re-plans from the arc so a post-plan title card is dropped on refine (re-inject).
- **Linked ADRs / items:** ADR-0019, ADR-0020, E-2.12, S-2.10.4/5/6/7/8, S-2.11.5b, S-2.12.1/2/3, ADR-0014.
