...
### D-055 — Open-ended refinement via two shared curation levers (E-2.12)
...
- **Consequences:** New Epic E-2.12 (S-2.12.1/2/3) + the S-2.10.x backlog landed; ADR-0019 (PlanDirective) + ADR-0020 (cast merge + stratified budget). One reservation-merge path (destination / refinement / feedback) and one shaping contract, so a fourth consumer (A-023 feedback) reuses them for free. Verified live: a pacing refinement re-rendered a child snapshot end-to-end. Follow-up: refinement re-plans from the arc so a post-plan title card is dropped on refine (re-inject).
