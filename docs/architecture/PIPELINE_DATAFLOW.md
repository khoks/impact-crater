...
**Flow.** Joins Stage 1+2+3 outputs into internal `_Asset`s, optionally annotates visible cast (A-018), then runs a six-step funnel: **(0)** drop `explicit` safety frames; **(1)** quality floor (`quality_score < 0.4` drop); **(2)** pHash near-dup collapse (Hamming ≤5, keep top `⌈n/dedup_factor⌉`); **(2b)** semantic best-of-burst dedup (A-017: L2-normalized embeddings, cosine ≥0.93 within 120s window; demands 0.97 when timestamps missing); **(3)** location/time clustering (`time_of_day|location_description` buckets) downsampled to 10/bucket; **(4)** rank by `combined_score = α·quality + β·narrative + γ·diversity` (defaults 0.3/0.5/0.2, diversity=1/cluster_size); **(5)** top-K to `target_size` within a floor (≥50 or 2×target_seconds) and ceiling (80% of input). Every drop/keep is logged for the transparency UI. Empties raise `Stage4EmptyCandidateSet` with funnel stats.
...
| Rank | `_combined_score` | deterministic | α/β/γ=0.3/0.5/0.2 | assets → ranked | overridable weights |
...
**Flow.** Walks `selected_items` by `placement_position`, resolving each `candidate_ref` back to a `MediaRecord` (photo or specific video scene; a `candidate_refs` fallback list recovers Opus's occasional short-integer refs). For each clip it derives `kind`, `source_path`, video `start/end_seconds`, `intended_duration_ms`, and `aspect_ratio_action` (photos: within ±10% of 16:9 → `as_is` else `smart_crop`; videos: ≈16:9 `as_is`, narrower `letterbox`, wider `pad`). Durations are then scaled: **standard mode** linear-scales to target (±10% tolerance, photos stretch, video capped at natural length, deficit redistributed across photos); **music-video mode** snaps every clip boundary to the nearest `CutGrid` cut ≥ floor (prev+250ms). The `snapshot_id` is minted here and the plan is written to `plan.json` + DB.
