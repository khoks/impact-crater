...
1. **Client / UI** — the single screen that carries the whole one-click promise: the user drops their media, types a free-text brief describing the video(s) and target platform(s), and clicks Submit — then later returns to review the preview and approve the publish. That preview-and-approve gate is the one intentional pause; everything between Submit and preview happens automatically. **TypeScript + React** served from the FastAPI process per [ADR-0005](./ADR-0005-process-topology-language-stack.md).
...
4. **Curation engine** — the 9-stage pipeline from per-asset metadata to a rendered Story Video, run automatically behind the single Submit (the user never operates these stages). Stages: ingest → bulk per-asset ops → rich metadata → pre-filter (floor + ≤80% ceiling) → narrative-arc judgment (N-001, Tier-L Opus) → plan compilation + orchestrator second-guess (with user reconfirm) → render → preview → agentic refinement (N-009, Tier-M tool-call loop). See [ADR-0011](./ADR-0011-curation-engine-algorithm.md).
...
6. **Agent harness** — single `Orchestrator` on Tier-M Sonnet 4.7 with a consolidated tool surface (LLM ops + pipeline + refinement + music + connector + person-library + profile tools). It runs automatically behind the single Submit and is not operated by the user — it drives the whole job on their behalf. Reasoning model = tool-call loop bounded at 50 turns/session; failure-mode UX = continue / abandon / restart. **Cross-project user profile (N-010)** persisted at `~/.impact-crater/profile/` provides agentic learning across the user's lifetime use of the product. See [ADR-0014](./ADR-0014-agent-harness-topology.md).
...
- **Ultimate Trip Package (future capability).** A planned future mode in which a single Submit yields a complete, multi-artifact set from one batch — per-location reels, albums, a full-journey music-scored video, and montages — assembled together by the same orchestrator + curation engine rather than one job at a time. Out of MVP scope; tracked as future roadmap work. The snapshot substrate (N-003), cross-project cache (N-007), and person library / trip cast (N-008/N-010) are the building blocks it will reuse.
