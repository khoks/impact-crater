...
> **Last updated:** 2026-06-30 — **AI-written splash-card title shipped (S-2.11.7, [PR #69](https://github.com/khoks/impact-crater/pull/69), D-055).** P3 polish follow-up to S-2.11.5: when the user doesn't type a `title_text`, the opt-in title card's auto-derived title now comes from a cheap Tier-S `generate_title_text` op (brief + modal year → clean 2–5 word title, place-name typo-fixing) that **reuses the structured-text primitive** (`parse_user_brief` + `{title}` schema) rather than widening the `LLMClient` protocol; the old first-clause heuristic is kept as the fail-soft fallback, and an explicit `title_text` short-circuits the call. New routing op + prompt (`prompts/generate_title_text/`), `stage6_title_card._title_from_brief`/`_clean_title`, 6 new unit tests (**backend suite 425 pass**), ruff/mypy clean. `PIPELINE_DATAFLOW.md` Stage 6.x refreshed. **Knowledge swept:** D-055, A-026 (future top-locations enrichment, gated on S-2.10.5).
...
| [E-2.10](./epics/E-2.10-feedback-driven-curation-quality.md) | Feedback-driven curation quality — coverage, balance, video, specialness (S-2.10.1/2/3 done; S-2.10.4/5 open) | Epic | P0 | mvp |
